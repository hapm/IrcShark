...
public class Scanner {
    ...
    Token NextToken() {
        ...
        switch (state) {
			...
			case 2:
				if (ch >= '!' && ch <= '"' || ch >= '$' && ch <= '%' || ch >= '(' && ch <= ')' || ch == ',' || ch >= '/' && ch <= ':' || ch == '=' || ch >= 'A' && ch <= '[' || ch == ']' || ch >= 'a' && ch <= '{' || ch == '}') {AddCh(); goto case 2;}
				else {t.kind = 2; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 3:
				if (ch <= 9 || ch >= 11 && ch <= 12 || ch >= 14 && ch <= 31 || ch >= '!' && ch <= 39 || ch >= '*' && ch <= '9' || ch >= ';' && ch <= 65535) {AddCh(); goto case 3;}
				else {t.kind = 3; break;}
			case 4:
				if (ch <= 8 || ch >= 11 && ch <= 12 || ch >= 14 && ch <= 31 || ch >= '!' && ch <= 39 || ch >= '*' && ch <= '9' || ch >= ';' && ch <= 65535) {AddCh(); goto case 4;}
				else {t.kind = 4; break;}
			case 5:
