...
namespace IrcShark.Extensions.Terminal
{
    ...
    public class ConsoleTerminal : ITerminal
    {
        ...
        /// </summary>
        Queue<WriteRequest> waitForWrite;
        ...
        /// </summary>
        /// <remarks>
        /// The ConsoleTerminal can only write full lines at a time, but if you want to 
        /// write lines with multible color, you need to use the Write methods what doesn't end
        /// a line automatically. Therefor all calls to Write are cached and are done, when an 
        /// end line is written.
        /// </remarks>
        private struct WriteRequest
        {
            ...
            /// <param name="bgColor">The backgroundcolor to use.</param>
            public WriteRequest(string text, ConsoleColor fgColor, ConsoleColor bgColor)
            {
                Text = text;
        ...
        public ConsoleTerminal(int histsize)
        {
            ...
            waitForWrite = new Queue<WriteRequest>();
        ...
        public void Write(string text)
        {
            if (!text.Contains("\n"))
            {
                waitForWrite.Enqueue(new WriteRequest(text, foregroundColor, Console.BackgroundColor));
            }
            else
            {
                ...
                string lastText = lines[lines.Length-1];
        ...
        private void WriteWaitingRequests()
        {
            WriteRequest request;
            ConsoleColor fgColor = Console.ForegroundColor;
            ConsoleColor bgColor = Console.BackgroundColor;
            while (waitForWrite.Count > 0) {
                request = waitForWrite.Dequeue();
                Console.ForegroundColor = request.ForegroundColor;
